 ...
 void event_handler_dropdown_btn(lv_event_t* e)
{
    ...
    if (code == LV_EVENT_CLICKED) {
        ...
        btn = lv_list_add_btn(lvgl_inst->dropdwn_list, LV_SYMBOL_FILE, "LED Effect");// create led button
        lv_obj_add_event_cb(btn, event_handler_dropdown_btn_opts_LED, LV_EVENT_CLICKED, (void*)lvgl_inst);
        btn = lv_list_add_btn(lvgl_inst->dropdwn_list, LV_SYMBOL_FILE, "ADC");// create ADC
        lv_obj_add_event_cb(btn, event_handler_dropdown_btn_opts_ADC, LV_EVENT_CLICKED, (void*)lvgl_inst);
 ...
 void event_handler_dropdown_btn_opts_LED(lv_event_t* e)
 {
	 lv_event_code_t code = lv_event_get_code(e);
	     Lvgl* const lvgl_inst = (Lvgl*)lv_event_get_user_data(e);
	     delete_dropdwn_list(lvgl_inst);
	     if (code == LV_EVENT_CLICKED) {
         static const char* btns[] = {"LED_E1","LED_E2","LED_E3","None",""};

         lv_obj_t* mbox1 = lv_msgbox_create(NULL, "LED Effect",NULL ,btns, true);
        lv_obj_add_event_cb(mbox1,event_handler_dropdown_LED,LV_EVENT_VALUE_CHANGED,NULL);
...
void event_handler_dropdown_LED(lv_event_t* e)
{
	 lv_obj_t * obj = lv_event_get_current_target(e);
	  LV_UNUSED(obj);
	uint16_t Btn_Pressed= lv_msgbox_get_active_btn(obj);
	switch (Btn_Pressed)
	{
	case 0:
	led_effect(1); break;
	...
	led_effect(3); break;
	case 3:
   led_effect_stop(); break;
	}


}
 void  event_handler_dropdown_btn_opts_ADC(lv_event_t* e)
 {
     ...
     delete_dropdwn_list(lvgl_inst);
     if (code == LV_EVENT_CLICKED) {
         lv_obj_t* mbox1 = lv_msgbox_create(NULL,"ADC", NULL, NULL, true);
         lv_obj_set_size(mbox1, 320,240);
         lv_obj_center(mbox1);
         lv_obj_t* arc = lv_arc_create(mbox1);
